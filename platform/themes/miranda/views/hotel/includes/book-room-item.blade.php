<div data-room-adults="{{$room->max_adults}}" data-room-children="{{$room->max_children}}" 
    ...
    <input data-room="{{$room->id}}" hidden name="numberOfRooms[{{$room->id}}]" type="number" value="{{ !isset($roomBookings[$room->id]) ? 0 : $roomBookings[$room->id]['number_of_rooms'] }}"/>
    <div class="d-flex flex-row" style="font-size: small; padding: 3px 5px; position: absolute; top: 15px; left: 15px;">
        ...
        <span class="rounded col d-flex align-items-center" style="background-color: black; color: white;">x<span data-qty-book="{{$room->id}}">{{ !isset($roomBookings[$room->id]) ? 0 : $roomBookings[$room->id]['number_of_rooms'] }}</span>&nbsp;room(s)</span>
        @if (!$checkout)
