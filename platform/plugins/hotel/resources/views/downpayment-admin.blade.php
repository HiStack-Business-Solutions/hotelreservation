...
<script>
    ...
    function onManualInputCheck() {
        ...
        if (!isManualInput) {
            var currencyInput = document.querySelector('#dp_amount_visible');
            currencyInput.value = "{{($booking->payment->dp_amount > 0 ? $booking->payment->dp_amount : $booking->amount * 0.5)}}";
            onBlurDP({target:currencyInput});
        }
    }
...
<div class="row p-2">
    ...
    <input class="col-sm-6 form-control" placeholder="Down Payment" disabled="true" name="dp_amount_visible" type="currency" value="{{($booking->payment->dp_amount > 0 ? $booking->payment->dp_amount : $booking->amount * 0.5)}}" id="dp_amount_visible" min="0" max="{{$booking->payment->amount}}">
</div>
